...
function openContentModal(type, id) {
    ...
    if (contentTypes[type]) {
        ...
        if (item) {
            ...
            if (viewsCount) viewsCount.textContent = interactions.views + 1;
...
function updateStorageUsage() {
    ...
    let totalSize = 0;
    Object.values(STORAGE_KEYS).forEach(key => {
        const data = JSON.stringify(getData(key));
        if (data) {
                ...
                if (processedData) {
                    ...
                    if (isEditMode && editId) {
                        ...
                        if (success) {
                            ...
                            showNotification(`Error updating ${config.displayName}`, 'error');
                        }
                    } else {
                        ...
                        if (success) {
                            ...
                            showNotification(`Error adding ${config.displayName}`, 'error');
                        }
                    }
                    this.reset();
                    updateAdminDashboard();
...
async function deleteItem(type, id) {
    ...
    const success = await saveData(config.storageKey, updatedItems);
